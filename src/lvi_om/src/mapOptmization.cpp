#include "utility.h"
#include "lvi_om/cloud_info.h"
#include "lvi_om/save_map.h"
// <!-- liorf_yjz_lucky_boy -->
...
public:
    ...
    mapOptimization()
    {
        ...
        pubKeyPoses = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/trajectory", 1);
        pubLaserCloudSurround = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/map_global", 1);
        pubLaserOdometryGlobal = nh.advertise<nav_msgs::Odometry>("lvi_om/mapping/odometry", 1);
        pubLaserOdometryIncremental = nh.advertise<nav_msgs::Odometry>("lvi_om/mapping/odometry_incremental", 1);
        pubPath = nh.advertise<nav_msgs::Path>("lvi_om/mapping/path", 1);

        subCloud = nh.subscribe<lvi_om::cloud_info>("lvi_om/deskew/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        subGPS = nh.subscribe<nav_msgs::Odometry>(gpsTopic, 200, &mapOptimization::gpsHandler, this, ros::TransportHints().tcpNoDelay());
        subLoop = nh.subscribe<std_msgs::Float64MultiArray>("lio_loop/loop_closure_detection", 1, &mapOptimization::loopInfoHandler, this, ros::TransportHints().tcpNoDelay());

        srvSaveMap = nh.advertiseService("lvi_om/save_map", &mapOptimization::saveMapService, this);

        pubHistoryKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/icp_loop_closure_history_cloud", 1);
        pubIcpKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/icp_loop_closure_corrected_cloud", 1);
        pubLoopConstraintEdge = nh.advertise<visualization_msgs::MarkerArray>("/lvi_om/mapping/loop_closure_constraints", 1);

        pubRecentKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/map_local", 1);
        pubRecentKeyFrame = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/cloud_registered", 1);
        pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("lvi_om/mapping/cloud_registered_raw", 1);

        pubSLAMInfo = nh.advertise<lvi_om::cloud_info>("lvi_om/mapping/slam_info", 1);
        pubGpsOdom = nh.advertise<nav_msgs::Path>("lvi_om/mapping/gnss_path", 100000);
    ...
    void laserCloudInfoHandler(const lvi_om::cloud_infoConstPtr &msgIn)
    {
        // extract time stamp
    ...
    void gpsHandler(const nav_msgs::Odometry::ConstPtr &gpsMsg)
    {
        if (!gps_initailized && (gpsMsg->pose.pose.position.x != 0 || gpsMsg->pose.pose.position.y != 0) && (gpsMsg->pose.covariance[0] < 0.003 && gpsMsg->pose.covariance[7] < 0.003))
        {
            ...
            cout << "GPS initailizes finish" << endl;
        }
        ...
        if (last_E != gpsMsg->pose.pose.position.x || last_N != gpsMsg->pose.pose.position.y)
        {
            ...
            tf::Matrix3x3(imu_quat).getRPY(imu_roll, imu_pitch, imu_yaw); // 进行转换
            Pwl = Pwi + Qwi.matrix() * Pil;
            msg_gnss_pose.header.frame_id = mapFrame;
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool saveMapService(lvi_om::save_mapRequest &req, lvi_om::save_mapResponse &res)
    {
    ...
    void visualizeGlobalMapThread()
    {
        ...
        lvi_om::save_mapRequest req;
        lvi_om::save_mapResponse res;
    ...
    void updateInitialGuess()
    {
        ...
        {
            transformTobeMapped[0] = initialPose[3];
            transformTobeMapped[1] = initialPose[4];
            transformTobeMapped[2] = initialPose[5];
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        for (int i = 0; i < laserCloudSelNum; i++)
        {
            ...
            float arx = (-srx * cry * pointOri.x - (srx * sry * srz + crx * crz) * pointOri.y + (crx * srz - srx * sry * crz) * pointOri.z) * coeff.x + (crx * cry * pointOri.x - (srx * crz - crx * sry * srz) * pointOri.y + (crx * sry * crz + srx * srz) * pointOri.z) * coeff.y;

            float ary = (-crx * sry * pointOri.x + crx * cry * srz * pointOri.y + crx * cry * crz * pointOri.z) * coeff.x + (-srx * sry * pointOri.x + srx * sry * srz * pointOri.y + srx * cry * crz * pointOri.z) * coeff.y + (-cry * pointOri.x - sry * srz * pointOri.y - sry * crz * pointOri.z) * coeff.z;

            float arz = ((crx * sry * crz + srx * srz) * pointOri.y + (srx * crz - crx * sry * srz) * pointOri.z) * coeff.x + ((-crx * srz + srx * sry * crz) * pointOri.y + (-srx * sry * srz - crx * crz) * pointOri.z) * coeff.y + (cry * crz * pointOri.y - cry * srz * pointOri.z) * coeff.z;
